...
Once the `conda-forge` channel has been enabled, `faiss, faiss-cpu, faiss-proc, libfaiss` can be installed with `conda`:

```
conda install faiss faiss-cpu faiss-proc libfaiss
```

or with `mamba`:

```
mamba install faiss faiss-cpu faiss-proc libfaiss
```
